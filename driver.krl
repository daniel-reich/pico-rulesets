ruleset driver {
  ...
  rule receive_contract_rumor {
    select when driver contract_rumor
    pre {
      orderID = event:attrs["Message"]["orderID"] // make sure store is sending this, needs to be unique (generate uuid)
      store_Rx = event:attrs["Message"]["store_Rx"] // make sure store is sending this
      lat = event:attrs["Message"]["location"]["latitude"]
      long = event:attrs["Message"]["location"]["longitude"]
      driver_lat = event:attrs["Message"]["driver_location"]["latitude"]
      driver_long = event:attrs["Message"]["driver_location"]["longitude"]
    }
    // make sure this event is actually being sent to a real rule
    ...
    event:send({
          ...
          "attrs": {
              "driver_Rx": meta:eci,
              "distance": dist:get_distance(driver_lat, driver_long, lat, long),
              "orderID": orderID
          }
      })
  }
...
// gossip protocol. move to own file: https://raw.githubusercontent.com/daniel-reich/pico-rulesets/master/driver_gossip.krl    
